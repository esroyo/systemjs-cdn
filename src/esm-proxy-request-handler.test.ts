import { assertEquals, assertSpyCallArg, returnsNext, stub } from '../dev_deps.ts';
...
Deno.test('esmProxyRequestHandler', async (t) => {
  ...
  await t.step('should forward the request to $esmOrigin keeping the parameters', async () => {
    const fetchStub = stub(_internals, 'fetch', returnsNext([ fetchReturn() ])); 
  ...
  await t.step('should forward the request to $esmOrigin removing the $basePath', async () => {
  ...
  await t.step('should replace the $esmOrigin by the self host', async () => {
    ...
    const res = await esmProxyRequestHandler(req);
    const systemjsCode = await res.text();
    assertEquals(!!systemjsCode.match(new RegExp(`${SELF_ORIGIN}/stable/vue@3.3.2/es2022/vue.mjs`)), true);
    fetchStub.restore();
  });

  await t.step('should replace the $esmOrigin by the self host including $basePath', async () => {
    Deno.env.set('BASE_PATH', '/sub-dir/234');
    const fetchStub = stub(_internals, 'fetch', returnsNext([ fetchReturn() ])); 
    const req = new Request(`${SELF_ORIGIN}/vue`);
    const res = await esmProxyRequestHandler(req);
    const systemjsCode = await res.text();
    assertEquals(!!systemjsCode.match(new RegExp(`${SELF_ORIGIN}/sub-dir/234/stable/vue@3.3.2/es2022/vue.mjs`)), true);
    fetchStub.restore();
    Deno.env.set('BASE_PATH', '');
  });

  await t.step('should use curl to get the "Location:" header if ESM_SEVICE_HOST response is not ok and redirect', async () => {
    const fetchStub = stub(_internals, 'fetch', returnsNext([
      Promise.resolve(new Response('', { status: 302 })),
    ]));
    const curlStub = stub(_internals, 'curl', returnsNext([
      Promise.resolve({ statusCode: 302, headers: [{ name: 'location', value: `${ESM_ORIGIN}/stable/vue@3.3.2`}] }),
    ]));
    const req = new Request(`${SELF_ORIGIN}/vue`);
    const res = await esmProxyRequestHandler(req);
    assertEquals(res.status, 302); 
    assertEquals(res.headers.get('location'), `${SELF_ORIGIN}/stable/vue@3.3.2`);
    fetchStub.restore();
    curlStub.restore();
  });

  await t.step('should use curl to get the "Location:" header if ESM_SEVICE_HOST response is not ok and pass-through if not redirecting', async () => {
    const fetchStub = stub(_internals, 'fetch', returnsNext([
