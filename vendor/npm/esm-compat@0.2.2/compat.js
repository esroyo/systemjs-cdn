import uaParser from "npm:ua-parser-js@legacy";
...
// From: github.com/evanw/esbuild@v0.25.9/internal/compat/js_table.go
const jsTable = {
  ...
  DynamicImport: {
    ...
    Firefox: [67, 0, 0],
    IOS: [11, 0, 0],
    Node: [13, 2, 0], // Note: Complex range in Go - simplified to latest supported
    Opera: [50, 0, 0],
    Safari: [11, 1, 0]
  },
  ExponentOperator: {
  ...
  ImportAssertions: {
    ...
    Deno: [1, 17, 0],
    Edge: [91, 0, 0],
    Node: [16, 14, 0] // Note: Complex range in Go - using start of supported range
  },
  ImportAttributes: {
    Chrome: [123, 0, 0],
    Deno: [1, 37, 0],
    Edge: [123, 0, 0],
    Firefox: [138, 0, 0], // Updated from Go source
    IOS: [17, 2, 0],
    Node: [20, 10, 0], // Note: Complex range in Go - using latest supported
    Opera: [109, 0, 0],
    Safari: [17, 2, 0]
  },
  ImportDefer: {},
  ...
  NodeColonPrefixImport: {
    ES: [0, 0, 0],
    Node: [14, 13, 1] // Note: Complex range in Go - using latest supported
  },
  NodeColonPrefixRequire: {
    ES: [0, 0, 0],
    Node: [16, 0, 0] // Note: Complex range in Go - using latest supported
  },
  NullishCoalescing: {
  ...
  RegexpUnicodePropertyEscapes: {
    ...
    ES: [2018, 0, 0],
    Firefox: [134, 0, 0], // Updated from Go source
    Node: [23, 3, 0] // Note: Complex range in Go - using latest supported
  },
  RestArgument: {
...
const getUnsupportedFeatures = (name, versionStr) => {
  ...
  return features.filter((feature) => {
    ...
    return versionLargeThan(v, version);
  });
};

const versionLargeThan = (v1, v2) => {
...
const getBuildTargetFromUA = (userAgent) => {
  ...
  if (userAgent.startsWith("Deno/")) {
    ...
    if (v) {
      const version = v.slice(1).map((v2) => parseInt(v2, 10));
  ...
  if (userAgent === "undici" || userAgent.startsWith("Node.js/") || userAgent.startsWith("Node/") || userAgent.startsWith("Bun/")) {
    return "node";
  }
...
const getEsmaVersionFromUA = (userAgent) => {
